...
\qss{}{Is the set ${x-}$axis${\setminus\{\text{Origin}\}}$ a closed set}
\qsc{}{brug??}
\begin{sol}
	We have to take its complement and check whether that set is a open set i.e. if it is a union of open balls
\end{sol}
...
\begin{thmpf}By openness of $V$, $x\in B_r(u)\subset V$
	\begin{center}
		\begin{tikzpicture}
...
\lem{}{Suppose $\vec{v_1}, \dots, \vec{v_n} \in \RR[n]$ is subspace of $\RR^n$.}
\begin{gpf}
	brug
\end{gpf}
\mprop{}{$1 + 1 = 2$.}
...
Now exercise
\qss{}{\label{exs1}Prove that triangle inequality is true if $p\geq 1$ for $p-$norms. (What goes wrong for $p<1$ ?)}
\begin{sol}
(\textbf{For Property \ref{n:3} for norm-2})	\subsubsection*{\textbf{When field is $\bbR:$}} We have to show\begin{align*}
